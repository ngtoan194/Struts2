										...
										<div class="wrapper line1">
											...
											<c:forEach items="${listphong }" var="p">
												<div class="col3">
													...
													<p class="pad_bot1">
														<strong class="color3">${p.tenPhong }</strong>
													</p>
													<p>	Số người: ${p.slnguoi }<br>
														Số phòng: ${p.slphong }<br>
														<f:formatNumber value="${p.gia }" currencySymbol="$" type="currency"/>
													</p>
													<p>${p.gioithieu }.</p>
													<s:url var="getRoomDetailURL" value="roomdetail.html">
														<s:param name="maPhong">${p.maPhong }</s:param>
													</s:url> 
												
													<s:a cssClass="button2" href="%{getRoomDetailURL}">Xem thêm</s:a>
												</div>
											</c:forEach>
											<!-- for -->
